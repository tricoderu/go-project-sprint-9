...
func Generator(ctx context.Context, chIn chan<- int64, fn func(i int64)) {
	...
	defer close(chIn) // Закрываем канал при завершении контекста (время на обработку истекло). Вызов close(chIn) позволяет обрубить все действия, связанные с этим каналом. Это предотвращает ситуацию, когда горутины "зависают", ожидая данные, которые больше не будут поступать.
...
func Worker(in <-chan int64, out chan<- int64) {
	// 2. Функция Worker
	// ...
	defer close(out)    // Закрываем out после завершения работы
...
func main() {
	...
	// Использовал атомарный подход, так как считаю его более легковесным (нет блокировки и последующей разблокировки) для простых операций, так как он не требует блокировки всех потоков на время выполнения операции
	go Generator(ctx, chIn, func(i int64) {
		atomic.AddInt64(&inputSum, i)   // атомарно увеличиваем значение переменной inputSum на значение, полученное в качестве параметра i
		atomic.AddInt64(&inputCount, 1) // атомарно увеличиваем значение переменной inputCount на 1. Это позволяет отслеживать, сколько раз была вызвана эта анонимная функция
	})
